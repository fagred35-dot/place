...
local function SetFly(v)
    ...
    if v then
        ...
        Connections.Fly = RunService.RenderStepped:Connect(function()
            local root=GetRoot(false); local hum=GetHum(false); local cam=workspace.CurrentCamera
            ...
            if S.SafePlatform and not S.Platform then
                -- auto create temp invisible platform below when not moving to prevent fall if fly disabled
            end
...
local function SetFreecam(v)
    ...
    if v then
        ...
        Connections.Freecam=RunService.RenderStepped:Connect(function()
            local move=GetMoveDir(Camera)*Runtime.FlySpeed*0.15
            FreecamCF = FreecamCF * CFrame.new(move) -- naive but lightweight
            -- mouse look is automatic via camera CFrame? We keep last
...
MakeBtn("🔭 FOV +10 (широкий обзор)", nil, function() local cam=workspace.CurrentCamera; if cam then cam.FieldOfView=math.min(120,cam.FieldOfView+10); SetStatus("FOV: "..cam.FieldOfView) end end)
MakeBtn("🔬 FOV -10 (узкий)", nil, function() local cam=workspace.CurrentCamera; if cam then cam.FieldOfView=math.max(30,cam.FieldOfView-10); SetStatus("FOV: "..cam.FieldOfView) end end)
MakeBtn("🎥 Reset FOV", nil, function() local cam=workspace.CurrentCamera; if cam then cam.FieldOfView=Saved.FOV end end)
...
MakeBtn("👁️ Spectate - смотреть за игроками (рандом)", nil, function()
    ...
    if S.Spectating==target then
        S.Spectating=nil; Camera.CameraSubject=GetHum(false); SetStatus("Spectate OFF")
    else
        S.Spectating=target; Camera.CameraSubject=target.Character.Humanoid; SetStatus("Spectate: "..target.Name.." (нажми снова чтобы выкл)")
    end
end)
